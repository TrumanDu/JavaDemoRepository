...
public class RedisMemCalculate {
	private static final long redisObject = 8 + 8;
	// 一个dictEntry，24字节，jemalloc会分配32字节的内存块
	private static final long dicEntry = 2 * 8 + 8 + 8;
	...
	public static long caculateBucketSzie(long size) {
		...
		double n = Math.log(size - remainder) / Math.log(2);
		double newSize = Math.pow(2, Math.ceil(n));
		return new Double(newSize).longValue();
	}
